...
### 定義
 ...
 2 | 変動日 | `transaction_date` | DATE | - | - | - | ○ | - | 変動が発生した日付 |
 3 | 変動額 | `transaction_amount` | INT | - | - | - | ○ | - | 符号なし整数 (常に正の数) |
 4 | 変動種別 | `transaction_type` | VARCHAR | 10 | - | - | ○ | - | `'income'`: 収入 / `'expense'`: 支出 |
...
### インデックス
* `idx_transactions_date`: `transaction_date` カラム（日付順の並び替え・日付範囲検索の高速化のため）
